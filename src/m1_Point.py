...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distance_traveled = self.distance_traveled + math.sqrt(((x - self.x) ** 2) + ((y - self.y) ** 2))
        self.x = x
        self.y = y
        self.moves_made += 1

    def move_by(self, dx, dy):
        self.distance_traveled = self.distance_traveled + math.sqrt((((self.x + dx) - self.x) ** 2) + (((self.y + dy) - self.y) ** 2))
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves_made += 1
    ...
    def get_distance_from_start(self):
        distance = math.sqrt(((self.start_x - self.x) ** 2) + ((self.start_y - self.y) ** 2))
        return distance
    ...
    def closer_to(self, point1, point2):
        distance_1 = math.sqrt(((point1.x - self.x) ** 2) + ((point1.y - self.y) ** 2))
        distance_2 = math.sqrt(((point2.x - self.x) ** 2) + ((point2.y - self.y) ** 2))
    ...
    def halfway_to(self, point):
        midpoint = ((point.x + self.x)/2), ((point.y + self.y)/2)
        return midpoint
